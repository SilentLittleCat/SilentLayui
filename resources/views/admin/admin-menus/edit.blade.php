        ...
        <div class="layui-col-sm8 layui-col-sm-offset2">
            <form class="layui-form" method="POST" action="{{ $pre_uri . 'update' }}">
                    ...
                    <div class="layui-input-block">
                        <select name="data[fa_id]" required lay-verify="required">
                            @foreach($model->getFatherMenuOptions() as $value)
                                <option value="{{ $value['id'] }}" {{ $item->fa_id == $value['id'] ? 'selected' : '' }}>{{ $value['name'] }}</option>
                            @endforeach
                        </select>
                        @if($errors->has('fa_id'))
                ...
                <div class="layui-form-item {{ $errors->has('icon') ? 'has-error' : '' }}">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="data[icon]" value="{{ isset(old('data')['icon']) ? old('data')['icon'] : $item->icon }}" placeholder="请输入图标" autocomplete="off" class="layui-input">
                        @if($errors->has('icon'))
                ...
                <div class="layui-form-item {{ $errors->has('link') ? 'has-error' : '' }}">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="data[link]" value="{{ isset(old('data')['link']) ? old('data')['link'] : $item->link }}" required lay-verify="required" placeholder="请输入链接" autocomplete="off" class="layui-input">
                        @if($errors->has('link'))
                ...
                <div class="layui-form-item {{ $errors->has('show') ? 'has-error' : '' }}">
                    ...
                    <div class="layui-input-block">
                        <input type="radio" name="data[show]" value="1" title="是" {{ $item->show != 2 ? 'checked' : '' }}>
                        <input type="radio" name="data[show]" value="2" title="否" {{ $item->show == 2 ? 'checked' : '' }}>
                        @if($errors->has('show'))
                ...
                <div class="layui-form-item {{ $errors->has('sort') ? 'has-error' : '' }}">
                    ...
                    <div class="layui-input-block">
                        <input type="number" name="data[sort]" value="{{ $item->sort }}" required lay-verify="required|integer" placeholder="请输入排序（大于等于0的整数）" autocomplete="off" class="layui-input">
                        @if($errors->has('sort'))
